    ...
    for fname in files:
        ...
        with container:
            with ui.element("div").classes(_ROW).style(f"padding-left: {indent_px}px"):
                ...
                tooltip = tr("di.tree_low_content_tooltip") if is_low_content else (
                    tr("di.tree_file_tooltip") if on_open_file is not None else ""
                )
                if tooltip:
    ...
    *,
    ...
    with container:
        with ui.element("div").classes(_ROW).style("padding-left: 4px"):
            ...
            ui.button(tr("di.tree_root"), on_click=_select_root).props(
