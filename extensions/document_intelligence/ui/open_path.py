# -*- coding: utf-8 -*-
"""Open local paths from Document Intelligence UI."""
...
def open_local_path(path: str) -> None:
    ...
    if not abspath or not os.path.exists(abspath):
        ui.notify(tr("di.open_path_not_found", path=path), color="warning")
        return
    try:
        ...
        ui.notify(tr("di.open_path_failed", error=exc), color="negative")
