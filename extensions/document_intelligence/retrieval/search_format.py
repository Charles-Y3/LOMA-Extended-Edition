...
from extensions.document_intelligence.index.text_match import (
    Significant,
    _MIN_COVERAGE,
...
def format_no_hits_message(
    *,
    ...
    lines = [tr("di.search_no_hits")]
    if task == "search":
        lines.append(tr("di.search_no_hits_try_ask"))
    if mode == "fast":
        lines.append(tr("di.search_no_hits_try_deep"))
    if semantic_suggest:
        lines.append(tr("di.search_no_hits_try_semantic"))
    return "\n\n".join(lines)
...
def format_search_chat(
    ...
    *,
    ...
    n = len(rows)
    lines = [
        tr("di.search_header", count=n),
        "",
        tr("di.search_query", query=query),
        "",
    ]
    if not rows:
