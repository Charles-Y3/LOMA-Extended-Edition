...
def remember_password(file_path: str, password: str) -> None:
    ...
    caches it for this run and persists it to Document Intelligence settings so
    ...
    try:
        from extensions.document_intelligence.settings import (
            load_settings,
            save_settings,
        )
...
def _open_password_dialog(req: _PasswordRequest) -> None:
    ...
    def _show() -> None:
        ...
        try:
            ui.notify(
                tr("di.pwd_notify", name=req.basename),
                color="warning",
                timeout=0,
                close_button=True,
            )
        ...
        try:
            with ui.dialog().props("persistent") as dlg, ui.card().classes(
                ...
                ui.label(tr("di.pwd_title")).classes("text-sm font-medium")
                ui.label(req.basename).classes("text-[11px] text-gray-400 break-all")
                ui.label(tr("di.pwd_paused")).classes("text-[11px] text-amber-300/90")
                pwd_in = ui.input(tr("di.pwd_input")).props(
                ...
                with ui.row().classes("w-full justify-end gap-2"):
                    ui.button(tr("di.pwd_skip"), on_click=_cancel).props("dense flat")
