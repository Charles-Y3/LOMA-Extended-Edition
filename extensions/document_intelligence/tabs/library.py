...
from extensions.document_intelligence.corpus.library import (
...
from extensions.document_intelligence.corpus.types import LibraryManifest
from extensions.document_intelligence.ui.browse import browse_folder
from extensions.document_intelligence.ui.corpus_panel import render_corpus_panel
...
def _index_chips(manifest: LibraryManifest, *, indexing: bool = False) -> str:
    ...
    if manifest.lexical_ready:
        parts.append(tr("di.lib_lexical_ok"))
    elif indexing:
        parts.append(tr("di.lib_lexical_busy"))
    if manifest.semantic_ready:
        parts.append(tr("di.lib_semantic_ok"))
    elif manifest.semantic_building or manifest.semantic_enabled:
        parts.append(tr("di.lib_semantic_busy"))
...
def render_library_tab() -> None:
        ...
        with ui.row().classes("w-full gap-2 shrink-0 items-center"):
            ...
            def _open_create_dialog() -> None:
                with ui.dialog() as dlg, ui.card().classes("gap-3 p-4 min-w-[280px]"):
                    ui.label(tr("di.lib_new_catalogue")).classes("text-sm font-medium")
                    name_in = ui.input(tr("di.lib_catalogue_name")).props("dense outlined dark").classes(
                        "w-full text-xs"
                    )

                    async def _confirm() -> None:
                        folder = await asyncio.to_thread(
                            browse_folder, title=tr("di.lib_select_folder")
                        )
                        if not folder or not os.path.isdir(folder):
                            ui.notify(tr("di.lib_folder_required"), color="warning")
                            return
                        lib = create_library(
                        ...
                        _refresh_libs()
                        _show_detail()
                        ui.notify(tr("di.lib_created", name=lib.name), color="positive")

                    with ui.row().classes("w-full justify-end gap-2"):
                        ui.button(tr("common.cancel"), on_click=dlg.close).props("dense flat")
                        ui.button(tr("di.lib_create"), on_click=_confirm).props(
            ...
            ui.button(icon="add", on_click=_open_create_dialog).props(
                "dense outline"
            ).classes("shrink-0 self-stretch").tooltip(tr("di.lib_new_tooltip"))
    ...
    def _refresh_libs() -> None:
        libs = list_libraries()
        corp = get_library(state["open_id"]) if state.get("open_id") else None
        options = {
    ...
    def _confirm_delete(library_id: str, name: str) -> None:
        with ui.dialog() as dlg, ui.card().classes("gap-3 p-4 min-w-[280px]"):
            ui.label(tr("di.lib_delete_title", name=name)).classes("text-sm font-medium")
            ui.label(tr("di.lib_delete_body")).classes("text-[11px] text-gray-400")
            ...
            def _do_delete() -> None:
                ...
                schedule_on_ui(
                    lambda: ui.notify(tr("di.lib_deleted"), color="positive")
                )
    ...
    def _show_detail() -> None:
        ...
        with detail_host:
            if not lid:
                ui.label(tr("di.lib_select_or_create")).classes(
            ...
            with ui.row().classes("w-full items-center gap-2 shrink-0"):
                ...
                with ui.row().classes("ml-auto gap-2 shrink-0 items-center"):
                    ...
                    async def _add_folder() -> None:
                        folder = await asyncio.to_thread(
                            browse_folder, title=tr("di.lib_add_folder_title")
                        )
                        if not folder or not os.path.isdir(folder):
                            return
                        ...
                        _refresh_libs()
                        ui.notify(tr("di.lib_folder_merged"), color="info")

                    ui.button(tr("di.lib_add_folder"), on_click=_add_folder).props("dense").classes(
                        "text-xs"
                    )
                    if not (
                        ...
                        def _on_semantic_click(c=corp) -> None:
                            if not c.manifest.lexical_ready:
                                ui.notify(tr("di.lib_semantic_needs_lexical"), type="warning")
                                return
                            ...
                            def _on_semantic_done(error: str = "") -> None:
                                ...
                                def _apply() -> None:
                                    if error:
                                        ui.notify(tr("di.lib_semantic_failed", error=error[:200]), type="negative")
                                    _refresh_libs()
                                    _show_detail()
                                    state["rebuild"]()
            ...
            def _stat() -> str:
                ...
                if corp.indexing:
                    ...
                    parts = [
                        ...
                        if total
                        else f"{segs} · {pair_n} · {tr('di.lib_stat_indexing_scan')}",
                    ]
                else:
                    excluded = corp.excluded_file_count()
                    files_disp = (
                        tr("di.lib_files_excl", files=m.file_count, excluded=excluded)
                        if excluded
                        else m.file_count
                    )
                    parts = [
                ...
                if corp.semantic_building:
                    parts.append(tr("di.lib_semantic_build"))
                return " · ".join(parts)
            ...
            def _tick() -> None:
                ...
                if not corp.indexing:
                    ...
                    if err and err != shown_error["text"]:
                        ...
                        ui.notify(tr("di.lib_index_error", error=err), type="negative")
                    elif not err:
                        shown_error["text"] = ""
                if corp.indexing:
                    p = corp.index_progress
                    ...
                    if total > 0:
                        ...
                        prog.set_value(0)
                        prog_label.set_text(name or tr("di.lib_scanning"))
                else:
                    prog_row.set_visibility(False)
                    prog.set_value(0)
                    prog_label.set_text("")
