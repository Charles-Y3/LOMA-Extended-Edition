...
from extensions.document_intelligence.corpus.workspace import get_workspace, reset_workspace
    ...
    with ui.column().classes("w-full h-full min-h-0 gap-2 px-2 pt-2 pb-0"):
        ...
        with ui.row().classes("w-full items-center justify-between gap-2 shrink-0 flex-wrap"):
            ...
            with ui.column().classes("gap-0 flex-1 min-w-0"):
                ...
                status_lbl = ui.label(tr("di.ws_add_folder_begin")).classes(
            ...
            async def _add_folder() -> None:
                ...
                path = await asyncio.to_thread(
                    ...
                    browse_folder, title=tr("di.ws_select_folder")
                ...
                ui.notify(tr("di.ws_indexing_started"), color="info")
            ...
            ui.button(tr("di.ws_new_workspace"), on_click=_reset).props("dense flat").classes(

                "text-[11px] text-gray-400"

            )

            ui.button(tr("di.ws_add_folder"), on_click=_add_folder).props("dense").classes(
        ...
        def _status() -> str:
            ...
            if ws.indexing:
                ...
                return f"{segs} · {pairs} · {tr('di.lib_stat_indexing_scan')}"

            if ws.index_error:

                return tr("di.ws_error", error=ws.index_error)
            ...
            if ws.roots:
                ...
                return tr("di.ws_waiting")

            return tr("di.ws_add_folder_begin")
        ...
        def _chips() -> str:
            ...
            files_disp = (
                ...
                tr("di.lib_files_excl", files=ws.file_count, excluded=excluded)
