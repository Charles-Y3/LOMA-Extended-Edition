    ...
    *,
    ...
    def _show() -> None:
        ...
        with ui.dialog() as dlg, ui.card().classes("gap-3 p-4 min-w-[320px]"):
            ui.label(tr("di.locks_title")).classes(
                "text-sm font-medium"
            )
            ui.label(tr("di.locks_body")).classes("text-[11px] text-gray-400")
            for name in names:
            ...
            with ui.row().classes("w-full justify-end gap-2"):
                ...
                ui.button(tr("di.locks_continue"), on_click=_go).props("dense flat color=primary")
        dlg.open()
